...
async function loadDataFromGist() {
    ...
    try {
        ...
        // 云端没有文件：把本地直接保存上去，作为初始化
        if (!file || !file.content) {
            console.log('云端无数据文件，上传本地初始化数据');
        ...
        console.log('成功从云端加载数据，云端用户数:', Object.keys(cloudData.users || {}).length);
        
        // 获取本地数据（但不立即覆盖全局变量）
        let localData = null;
        try {
            const localDataStr = localStorage.getItem('leagueScoreData');
            if (localDataStr) {
                localData = JSON.parse(localDataStr);
                console.log('本地数据用户数:', Object.keys(localData.users || {}).length);
            }
        } catch (error) {
            console.log('本地数据解析失败，将使用云端数据');
        }

        // 进行数据融合
        const merged = mergeData(cloudData, localData);
        console.log('数据融合完成，融合后用户数:', Object.keys(merged.users).length);

        // 更新全局变量
        users = merged.users;
        predictions = merged.predictions;
        matchResults = merged.matchResults;
        leaderboard = merged.leaderboard;
        history = merged.history;
        currentRound = merged.currentRound;
        deadlines = merged.deadlines;
        scheduledMatches = merged.scheduledMatches;
        ...
        // 如果融合结果与云端不同，则把融合结果推送到云端
        const cloudUsersCount = Object.keys(cloudData.users || {}).length;
        const mergedUsersCount = Object.keys(merged.users).length;
        if (mergedUsersCount !== cloudUsersCount) {
            console.log('融合后用户数发生变化，推送到云端');
            await saveDataToGist();
        }
        ...
        console.log('数据已从GitHub Gist加载并融合完成');
    } catch (error) {
...
async function testSync() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // ✅ 不再在这里自动 saveDataToGist()，避免误覆盖
            // 如需对齐，请调用 forceSyncAllData()，它会先拉取再融合
            updateSyncStatusWithCloud();
        } else {
...
function updateSyncStatus() {
    ...
    if (syncStatusContainer) {
        // 检查是否已经有统计信息，如果没有则创建
        let statsElement = syncStatusContainer.querySelector('.data-stats');
        if (!statsElement) {
            ...
            syncStatusContainer.appendChild(statsElement);
        }
        
        // 更新统计信息
        statsElement.innerHTML = `
            <h4>当前数据统计</h4>
            <p>本地用户数: ${Object.keys(users).length}</p>
            <p>本地预测数: ${Object.keys(predictions).length}</p>
            <p>本地比赛结果数: ${Object.keys(matchResults).length}</p>
            <p>当前轮次: ${currentRound}</p>
        `;
    }
}
...
// 把任意 timestamp 转成 Date。非法/缺失时间 → 最小时间（更旧）
function toDate(ts) {
    const d = new Date(ts);
...
function mergeUserRecord(cloudUser = {}, localUser = {}) {
    ...
    for (const key of allKeys) {
        ...
        const cVal = cloudUser[key];
        const lVal = localUser[key];
        ...
        if (lVal === undefined) {
            ...
            // 双方都有 → 决策
            // 尝试基于字段自己的时间戳（如果该字段是个对象且带 timestamp/updatedAt）
            const cFieldTime = toDate(cVal?.timestamp || cVal?.updatedAt);
            const lFieldTime = toDate(lVal?.timestamp || lVal?.updatedAt);

            if (cFieldTime.getTime() !== 0 || lFieldTime.getTime() !== 0) {
                merged[key] = (lFieldTime > cFieldTime) ? lVal : cVal;
            } else {
                // 没有字段级时间 → 优先非空，再按整对象 updatedAt，最后本地优先
                if (lVal !== null && lVal !== '' && lVal !== undefined) {
                    merged[key] = lVal;
                } else if (cVal !== null && cVal !== '' && cVal !== undefined) {
                    merged[key] = cVal;
                } else {
                    merged[key] = preferLocalObject ? lVal : cVal;
                }
            }
        }
    }

    // updatedAt：取较新的时间（如果两侧都有，选大的；否则保留存在的）
    merged.updatedAt = (localUpdated > cloudUpdated)
        ? (localUser.updatedAt || cloudUser.updatedAt)
        : (cloudUser.updatedAt || localUser.updatedAt);
...
function matchKeyOf(match) {
    ...
    const teamPair = `${match.team1}_${match.team2}`;
    if (match.date || match.time || match.kickoff) {
        return `${teamPair}_${match.date || ''}_${match.time || match.kickoff || ''}`;
    }
    return teamPair;
}
...
// 智能数据融合函数
function mergeData(cloudData, localData = null) {
    const merged = {
        users: {},
        predictions: {},
        matchResults: {},
    ...
    if (!cloudData) {
        if (localData) {
            return {
                ...
                matchResults: { ...localData.matchResults },
                leaderboard: [...localData.leaderboard],
                history: { ...localData.history },
                currentRound: localData.currentRound,
                deadlines: { ...localData.deadlines },
                scheduledMatches: { ...localData.scheduledMatches }
            };
        } else {
            // 既没有云端数据也没有本地数据，返回空数据
            return merged;
        }
    }

    // 1) users：合并所有用户，本地用户覆盖云端用户（避免冲突）
    // 先添加云端的所有用户
    ...
    if (localData && localData.users) {
        for (const username in localData.users) {
            merged.users[username] = { ...localData.users[username] };
        }
    }
    ...
    const allUsernames = new Set([
        ...
        ...(localData ? Object.keys(localData.predictions || {}) : [])
    ]);
    
    for (const username of allUsernames) {
        merged.predictions[username] = {};
        const cloudPreds = cloudData.predictions?.[username] || {};
        const localPreds = localData?.predictions?.[username] || {};
        ...
        for (const matchKey of allMatches) {
            const c = cloudPreds[matchKey];
            const l = localPreds[matchKey];
            if (c && l) {
                merged.predictions[username][matchKey] =
                    toDate(l.timestamp) > toDate(c.timestamp) ? l : c;
            } else {
                merged.predictions[username][matchKey] = c || l;
            }
        }
    }

    // 3) matchResults：按 timestamp 取新
    const allMatchKeys = new Set([
        ...Object.keys(cloudData.matchResults || {}),
        ...(localData ? Object.keys(localData.matchResults || {}) : [])
    ]);
    
    for (const matchKey of allMatchKeys) {
        const c = cloudData.matchResults?.[matchKey];
        const l = localData?.matchResults?.[matchKey];
        merged.matchResults[matchKey] = (c && l)
            ? (toDate(l.timestamp) > toDate(c.timestamp) ? l : c)
            : (c || l);
    }

    // 4) leaderboard：同用户名合并 rounds，重算 totalScore
    ...
    for (const item of allLeaderboardItems) {
        ...
        if (!lbMap.has(item.username)) {
            lbMap.set(item.username, { ...item, rounds: [...(item.rounds || [])] });
        } else {
            const existing = lbMap.get(item.username);
            const roundsMap = new Map(existing.rounds.map(r => [r.round, r]));
            for (const r of (item.rounds || [])) {
                roundsMap.set(r.round, r); // 后来者覆盖同轮
            }
            existing.rounds = Array.from(roundsMap.values());
        }
    }
    merged.leaderboard = Array.from(lbMap.values())
        .map(u => ({
            ...u,
            totalScore: (u.rounds || []).reduce((s, r) => s + (r.score || 0), 0)
        }))
    ...
    if (localData && localData.history) {
        merged.history = { ...merged.history, ...localData.history };
    }

    // 6) currentRound：取较大
    merged.currentRound = Math.max(
        cloudData.currentRound || 1, 
        localData?.currentRound || 1
    );

    // 7) deadlines：合并（本地优先）
    merged.deadlines = { ...cloudData.deadlines };
    if (localData && localData.deadlines) {
        merged.deadlines = { ...merged.deadlines, ...localData.deadlines };
    }

    // 8) scheduledMatches：按轮去重合并，key 更稳（matchId > team1_team2_date > team1_team2）
    const allRounds = new Set([
        ...Object.keys(cloudData.scheduledMatches || {}),
        ...(localData ? Object.keys(localData.scheduledMatches || {}) : [])
    ]);
    
    for (const round of allRounds) {
        const cList = cloudData.scheduledMatches?.[round] || [];
        const lList = localData?.scheduledMatches?.[round] || [];
        const map = new Map();
        for (const m of cList) map.set(matchKeyOf(m), m);
        for (const m of lList) {
            const key = matchKeyOf(m);
            if (!map.has(key)) map.set(key, m);
            else {
                // 如果同时存在，保留较新的（按 timestamp 决策）
                const old = map.get(key);
                const newer = toDate(m?.timestamp) > toDate(old?.timestamp) ? m : old;
                map.set(key, newer);
            }
        }
        merged.scheduledMatches[round] = Array.from(map.values());
    }
        ...
        if (response.ok) {
            ...
            if (file && file.content) {
                ...
                return {
                    ...
                    predictions: Object.keys(cloudData.predictions || {}).length,
                    matchResults: Object.keys(cloudData.matchResults || {}).length,
                    currentRound: cloudData.currentRound || 1,
...
// 更新数据同步状态（包含云端数据对比）
async function updateSyncStatusWithCloud() {
    const statusElement = document.getElementById('syncStatus');
    const gistIdElement = document.getElementById('gistIdDisplay');
    ...
    if (syncStatusContainer) {
        ...
        let statsHTML = `
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                <div>
                    <h5>本地数据</h5>
                    <p>用户数: ${Object.keys(users).length}</p>
                    <p>预测数: ${Object.keys(predictions).length}</p>
                    <p>比赛结果数: ${Object.keys(matchResults).length}</p>
                    <p>当前轮次: ${currentRound}</p>
                </div>
        `;
        ...
        if (cloudStats) {
            statsHTML += `
                <div>
                    <h5>云端数据</h5>
                    <p>用户数: ${cloudStats.users}</p>
                    <p>预测数: ${cloudStats.predictions}</p>
                    <p>比赛结果数: ${cloudStats.matchResults}</p>
                    <p>当前轮次: ${cloudStats.currentRound}</p>
                    <p>最后更新: ${new Date(cloudStats.lastUpdated).toLocaleString('zh-CN')}</p>
                </div>
            `;
        } else {
            statsHTML += `
        ...
        // 添加同步建议
        if (cloudStats) {
            const localUserCount = Object.keys(users).length;
            const cloudUserCount = cloudStats.users;
            ...
            if (localUserCount !== cloudUserCount) {
                statsHTML += `
                    <div style="margin-top: 10px; padding: 10px; background-color: #fff3cd; border: 1px solid #ffeaa7; border-radius: 5px;">
                        <strong>⚠️ 数据不一致提醒</strong><br>
                        本地用户数 (${localUserCount}) 与云端用户数 (${cloudUserCount}) 不一致<br>
                        建议点击"强制同步所有数据"按钮进行数据融合
                    </div>
                `;
            } else {
                statsHTML += `
                    <div style="margin-top: 10px; padding: 10px; background-color: #d4edda; border: 1px solid #c3e6cb; border-radius: 5px;">
                        <strong>✅ 数据同步状态良好</strong><br>
                        本地数据与云端数据一致
                    </div>
                `;
            }
...
// 强制覆盖云端数据（用本地数据完全替换云端数据）
async function forceOverwriteCloudData() {
    if (!GIST_CONFIG.token) {
        alert('请先配置GitHub Token');
        return;
    }
    ...
    // 确认操作
    const confirmMessage = `⚠️ 警告：此操作将用本地数据完全覆盖云端数据！\n\n` +
        `本地数据统计：\n` +
        `- 用户数: ${Object.keys(users).length}\n` +
        `- 预测数: ${Object.keys(predictions).length}\n` +
        `- 比赛结果数: ${Object.keys(matchResults).length}\n` +
        `- 当前轮次: ${currentRound}\n\n` +
        `云端所有其他用户的数据将被删除！\n\n` +
        `确定要继续吗？`;
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (file && file.content) {
                const cloudData = JSON.parse(file.content);
                cloudPreview = {
                    users: Object.keys(cloudData.users || {}),
                    predictions: Object.keys(cloudData.predictions || {}),
                    matchResults: Object.keys(cloudData.matchResults || {}),
                    currentRound: cloudData.currentRound || 1
                };
                
                // 显示详细预览
                const previewMessage = `📋 强制覆盖预览：\n\n` +
                    `云端当前数据：\n` +
                    `- 用户: ${cloudPreview.users.join(', ') || '无'}\n` +
                    `- 预测数: ${cloudPreview.predictions.length}\n` +
                    `- 比赛结果数: ${cloudPreview.matchResults.length}\n` +
                    `- 当前轮次: ${cloudPreview.currentRound}\n\n` +
                    `本地数据将覆盖：\n` +
                    `- 用户: ${Object.keys(users).join(', ') || '无'}\n` +
                    `- 预测数: ${Object.keys(predictions).length}\n` +
                    `- 比赛结果数: ${Object.keys(matchResults).length}\n` +
                    `- 当前轮次: ${currentRound}\n\n` +
                    `⚠️ 云端用户 ${cloudPreview.users.filter(u => !users[u]).join(', ') || '无'} 将被永久删除！\n\n` +
                    `确定要执行强制覆盖吗？`;
        ...
        console.log('获取云端数据预览失败:', error);
        // 如果无法获取预览，继续执行
        if (!confirm('无法获取云端数据预览，确定要继续强制覆盖吗？')) {
            return;
        }
    }
    
    try {
        console.log('开始强制覆盖云端数据...');
        console.log('本地用户数:', Object.keys(users).length);
        console.log('本地用户列表:', Object.keys(users));
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (file && file.content) {
                    ...
                    cloudUsers = Object.keys(cloudData.users || {});
                    console.log('云端数据统计:', cloudStats);
                    console.log('云端用户列表:', cloudUsers);
                }
            }
        } catch (error) {
            console.log('获取云端数据统计失败:', error);
        }
        ...
        if (result) {
            ...
            const operationLog = {
                ...
                localUsers: Object.keys(users),
                cloudUsersBefore: cloudUsers,
                deletedUsers: cloudUsers.filter(u => !users[u]),
                localStats: {
            ...
            // 保存操作日志到本地存储
            let operationLogs = JSON.parse(localStorage.getItem('leagueScoreOperationLogs') || '[]');
            operationLogs.push(operationLog);
            localStorage.setItem('leagueScoreOperationLogs', JSON.stringify(operationLogs));
            
            console.log('操作日志已记录:', operationLog);
            
            // 显示覆盖结果对比
            let resultMessage = '✅ 强制覆盖云端数据成功！\n\n';
            resultMessage += '覆盖前后对比：\n';
            
            if (cloudStats) {
                resultMessage += `用户数: ${cloudStats.users} → ${Object.keys(users).length}\n`;
                resultMessage += `预测数: ${cloudStats.predictions} → ${Object.keys(predictions).length}\n`;
                resultMessage += `比赛结果数: ${cloudStats.matchResults} → ${Object.keys(matchResults).length}\n`;
                resultMessage += `当前轮次: ${cloudStats.currentRound} → ${currentRound}\n`;
                
                // 添加用户列表对比
                if (cloudUsers.length > 0) {
                    resultMessage += '\n被删除的云端用户：\n';
                    cloudUsers.forEach(username => {
                        if (!users[username]) {
            ...
            console.error('强制覆盖云端数据失败');
            alert('❌ 强制覆盖云端数据失败，请检查网络连接');
        }
    } catch (error) {
        console.error('强制覆盖云端数据出错:', error);
        alert('❌ 强制覆盖云端数据出错: ' + error.message);
    }
}

// 查看操作日志
function viewOperationLogs() {
    const logs = JSON.parse(localStorage.getItem('leagueScoreOperationLogs') || '[]');
    ...
    recentLogs.forEach((log, index) => {
        ...
        logMessage += `${index + 1}. ${date}\n`;
        logMessage += `   操作类型: ${log.type === 'forceOverwrite' ? '强制覆盖云端数据' : log.type}\n`;
        logMessage += `   操作者: ${log.operator}\n`;
        
        if (log.type === 'forceOverwrite') {
            logMessage += `   本地用户: ${log.localUsers.join(', ') || '无'}\n`;
            logMessage += `   被删除用户: ${log.deletedUsers.join(', ') || '无'}\n`;
            logMessage += `   本地统计: 用户${log.localStats.users}个, 预测${log.localStats.predictions}个, 比赛结果${log.localStats.matchResults}个\n`;
        }
    ...
    if (logs.length > 10) {
        logMessage += `... 还有 ${logs.length - 10} 条更早的记录\n`;
    }
    
    logMessage += '\n💡 提示：操作日志保存在本地，不会同步到云端';
